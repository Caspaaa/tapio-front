...
export function NewPostForm({ posts, setPosts }: NewPostFormProps): JSX.Element {
	const { showModal, setShowModalFalse } = useContext(ModalContext);
	...
	return (
		<>
			<div className={`fixed inset-0 flex justify-center items-center bg-gray-500 transition-opacity duration-500 ${showModal ? 'bg-opacity-10' : 'bg-opacity-0'}`}>
				<div className={`absolute w-full sm:w-3/4 xl:w-2/3 2xl:1/2 h-screen bg-white p-12 phone-xl:rounded-[4rem] transition-transform duration-50 ${showModal ? 'translate-y-40' : 'translate-y-full'}`}>
					<button onClick={() => setShowModalFalse()}>
					...
					<form onSubmit={createPost} className="space-y-4 pt-16 phone-xl:px-8 sm:px-12 xl:px-24 2xl:px-36">
						<div>
							...
							<input id="title" type="text" value={title} placeholder="Type the story title" onChange={(event) => setTitle(event.target.value)} className="border border-gray-50 py-2 px-4 text-gray-600 rounded-lg w-full focus:outline-none shadow" />
						</div>
						<div>
						...
						<div className="flex justify-end mt-4">
							<button type="button" onClick={() => setShowModalFalse()} className="bg-gray-50 border-gray-100 hover:bg-gray-100 hover:border-gray-300 text-gray-300 hover:text-gray-400 p-2 rounded-full mr-2 duration-200">
